    ...
    async load() {
        ...
        [
            ...
            [
                ...
                "player",
                "enemy",
            ].map((n) => loadImg(`../img/${n}.png`)),
        );

        this.obstacles = await Promise.all(
            Array.from({ length: 9 }, (_, i) =>
                loadImg(`../img/obstacle${i + 1}.png`),
            ),
        );

        const sfx = this.sfx;
        sfx.death = new Audio("../fx/death.wav");
        sfx.startlevel = new Audio("../fx/startlevel.wav");
        sfx.action = new Audio("../fx/action.wav");

        for (let i = 0; i < 3; i++) {
            const audio = new Audio("../fx/shot.wav");
            audio.load();
...
const Map = {
    ...
    async load(mapNum = null) {
        ...
        const res = await fetch(`../maps/map${mapNum}.json`);
